...
function validate() {
  ...
  document
    .getElementById("dependant-dob")
    .addEventListener("blur", validateDependantDOB);
  ...
  document
    .getElementById("dependant-ssn")
    .addEventListener("blur", validateSSN);
  ...
  document.getElementById("zipCode").addEventListener("input", formateZipCode);
  ...
  function validateCreditCardNumber() {
    ...
    if (
      !creditCardNumberRegex.test(creditCardNumberValue) &&
      !cardNumberRegex.test(creditCardNumberValue)
    ) {
  ...
  document.getElementById("cvvCode").addEventListener("blur", validateCVVCode);
  document
    .getElementById("cvvCode")
    .addEventListener("input", validateCVVFormat);
  document.getElementById("expDate").addEventListener("blur", validateExpDate);
  document
  ...
  document
    .querySelector("div.form-button:nth-child(5) > input:nth-child(2)")
    .addEventListener("click", validateAdditionalInfo);
  ...
  function validateAdditionalInfo(event) {
    ...
    if (selectedEligibility === "FPHA") {
      ...
      if (flag) {
        document.getElementById("elgibility-error").innerText =
          "Please Select an Housing Assitance option";
      } else {
        document.getElementById("elgibility-error").innerText = "";
      }
    } else if (
      ...
      if (!isNaN(householdSizeValue) && householdSizeValue !== "") {
        ...
        document.getElementById("elgibility-error").innerText =
          "Please Enter the correct House Hold size";
      }
    }
  ...
  function handleBlurLabel(e) {
    ...
    if (
      ...
      e.target.nextElementSibling.tagName === "SPAN"
    )
  ...
  document
    .getElementById("addressOnCard")
    .addEventListener("input", validateAddress);

  function validateAddress() {
    let inputField = this;
    const errorMessage = inputField.nextElementSibling;
    // Get the input value in lowercase for case-insensitive comparison
    let addressValue = inputField.value.toLowerCase();
    ...
    for (var i = 0; i < disallowedPhrases.length; i++) {
      let index = addressValue.indexOf(disallowedPhrases[i]);

      if (index !== -1) {
        // Remove the disallowed phrase and everything after it
        inputField.value = inputField.value.substring(0, index);
        errorMessage.innerText =
          "Address cannot contain 'P.O. Box' or 'PO Box'.";
        break;
      } else {
        // If the address is valid, clear the error message
        errorMessage.innerText = "";
      }
    }
  }
  ...
  document
    .querySelector('input[name="bqp"]')
    .addEventListener("click", showBqpDetails);
  ...
  document
    .querySelectorAll('input[name="bqp"]')[1]
    .addEventListener("click", hideBqpDetails);
